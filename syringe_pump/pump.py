...
from syringe_pump.serial_interface import PumpCommandError
...
class Pump(PumpSerial, AbstractAsyncContextManager):
    ...
    async def _initialise(self):
        ...
        try:
            ...
            #print(f"\033[93mWarning: Clock setting failed — continuing anyway. Error: {e}\033[0m")
            set_clock = False
    ...
    async def set_mode(self, mode: str = "iw"):
        """Set the pump to infusion, withdrawal, or both (iw)."""
        # 'iw' is non-specific, may be UI-only; we choose INF by default
        if mode == "i":
            await self._write("/1DIR INF", error_state_ok=True)
        elif mode == "w":
            await self._write("/1DIR WDR", error_state_ok=True)
        elif mode == "iw":
            # No explicit 'both' mode on hardware — you just change DIR before running
            # Optionally store this in your app logic
            pass
        else:
